...
async function _parseProjectInput(data) {
  ...
  if (!chapter) {
    throw new LGBadInputError(`Chapter not found for identifier ${chapterIdentifier}`)
  }
  ...
  if (userIdentifiers && users.length !== userIdentifiers.length) {
    ...
    throw new LGBadInputError(`Users not found for identifiers: ${notFoundIds.join(', ')}`)
  }

  const cycle = await getCycleForChapter(chapter.id, cycleIdentifier)
  if (!cycle) {
    throw new LGBadInputError(`Cycle not found for identifier ${cycleIdentifier} in chapter ${chapterIdentifier}`)
  }
  if (cycle.chapterId !== chapter.id) {
    throw new LGBadInputError(`Cycle ${cycleIdentifier} chapter ID ${cycle.chapterId} does not match chapter ${chapterIdentifier} ID ${chapter.id}`)
  }
  ...
  if (projectIdentifier) {
    ...
    if (project) {
      if (project.chapterId !== chapter.id) {
        throw new LGBadInputError(`Project ${projectIdentifier} chapter ID ${project.chapterId} does not match chapter ${chapterIdentifier} ID ${chapter.id}`)
      }
      if (project.cycleId !== cycle.id) {
        throw new LGBadInputError(`Project ${projectIdentifier} cycle ID ${project.cycleId} does not match cycle ${cycleIdentifier} ID ${cycle.id}`)
      }
    }
  }
  ...
  if (goalIdentifier && !goal) {
    ...
    if (!goal) {
      throw new LGBadInputError(`Goal not found with identifier: ${goalIdentifier}`)
    }
  }

  if (!project) {
    if (!goal) {
      throw new LGBadInputError('New project imports must specify a goal')
    }
    if (!Array.isArray(userIdentifiers) || userIdentifiers.length === 0) {
      throw new LGBadInputError('New project imports must specify at least one user')
    }
  }
